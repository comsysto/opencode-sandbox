...
**occ-sandbox** runs OpenCode inside a Docker container, giving each project its own isolated environment:
...
occ-sandbox uses [mise](https://mise.jdx.dev/) to manage software inside the container. **mise does not need to be installed on your host machine**, though using it on the host as well is recommended for a consistent toolchain experience.
...
That's it — the `occ-*` commands are now available globally.
...
| Situation | Command |
|---|---|
| First time setup | `occ-rebuild-container` then `occ-start-container` |
| Daily use | `occ-start-container` |
| After changing `mise.toml` | `occ-rebuild-container` then `occ-start-container` |
| Stop the container | `Ctrl+C` in the `occ-start-container` terminal |

> **Note:** The container is not removed on stop — state is preserved between sessions. `occ-rebuild-container` will remove the old container before building a new image.
...
occ-sandbox/
├── bin/
│   ├── occ-rebuild-container   # Build the Docker image
│   ├── occ-start-container     # Start / resume the container
│   ├── occ-terminal            # Attach a terminal session
│   ├── occ-web                 # Open the web UI
│   └── occ-web-auth            # Open the web UI with authentication
